...
class menu
{
  ...
  public $estructura;  //vector con la estructura para armar el menu
  public $nombre;

  public function __construct($estruct)
  {
  ...
  function mostrar($actual)
  {
    ...
    for ($i = 0; $i < count($this->estructura['items']); $i++) {
      ...
      if ($i == count($this->estructura['items']) - 1) $rightborder = 'border-right:solid 1px #000000;';
      else $rightborder = '';
      if ($i != $actual) {
        $colorc = $this->estructura['b_color_un'];
        $leftborder = 'border-bottom:solid 1px #000000;';
