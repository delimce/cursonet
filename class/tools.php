  ...
  /*  numero de registros, y solo 1 fila  NOTA: en el caso de un solo valor se puede utilizar el nombre de arreglo sin indice*/
   ...
   function buscar_estructdb($matriz, $clave, $elemento, $consultar)
   {
      ...
      for ($j = 0; $j < count($matriz); $j++) {
         ...
         if ($matriz[$j][$clave] == $elemento) {
            $a = $matriz[$j][$consultar];
            break;
         }
      }
   ...
   function repetidos($array)
   {
      ...
      if (count($repeat) != count($array)) return true;
      else return false;
   }
   ...
   static function redirect($url, $parent = '')
   {

      if ($parent != '') $parent .= '.';

      if ($url == "cerrar") $var = "window.close();";
      else $var = "$parent" . "location.replace('$url');";
   ...
   static function javaviso($aviso, $url = false)
   {
