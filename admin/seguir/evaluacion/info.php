...
$menu = new menu($menu_struct);
  ...
  $seccion = $pru->array_query("select grupo_id from evaluacion where id = '{$_GET['id']}'");
  if($seccion[0]>0) $filtro = "and grupo_id = $seccion[0]"; else $filtro = "";
  
  
  $query = " SELECT concat(e.nombre,' ',e.apellido,' - ',id_number) as nombre2,
  					ifnull((select id from evaluacion_estudiante where eval_id = '{$_GET['id']}' and est_id = e.id ),'NO') as presento,
					ifnull((select nota from evaluacion_estudiante where eval_id = '{$_GET['id']}' and est_id = e.id ),'NO') as revision
