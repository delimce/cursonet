...
if (isset($_POST['login12'])) {
    ...
    if ($crear->nreg > 0) {
        ...
        $crear->insertar2("tbl_admin", "nombre, apellido, user, pass, es_admin, email, telefono, fax, fecha, sintesis", $valores2);
        $newAdminId = $crear->getLastIdInserted();
        // add courses to admin
        $courses = (count($_POST['curso']) > 0) ? $_POST['curso'] : [];
        array_walk($courses, function ($id) use ($crear, $newAdminId) {
            $adminCourse[0] = $newAdminId;
            $adminCourse[1] = $id;
            $adminCourse[2] = date("Y-m-d H:i:s");
            $adminCourse[3] = date("Y-m-d H:i:s");
            $crear->insertar2("tbl_admin_curso", "admin_id,curso_id,created_at,updated_at", $adminCourse);
        });
                            ...
                            <form name="form1" method="post" action="crear.php">
                                <table width="100%" border="0" cellspacing="4" cellpadding="3">
                                    ...
                                    <tr>
                                        ...
                                        <td>&nbsp;</td>
                                        <td width="4%" valign="middle" class="style3"><input name="admin" type="checkbox" id="admin" value="1"></td>
                                    ...
                                    ?>
                                        <tr>
                                            <td width="4%" align="center" valign="top" class="style3"><input name="curso[]" type="checkbox" value="<?= $cursos[$j]['id'] ?>">
                                            </td>
                                            <td colspan="6">
                                                <span class="style3"><?= $cursos[$j]['nombre'] ?></span> <span class="small">
                                    ...
                                    <tr>
                                        <td colspan="7" align="center" class="style3"><br>
                                            ...
                                            <input type="button" id="submit3" name="Button" value="<?= LANG_save ?>" onClick="validar();">
                                            <br></td>
                                    </tr>
                                </table>
                            </form>
