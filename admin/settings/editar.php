...
if (isset($_REQUEST['ItemID'])) {
  ...
  $query = "select nombre,apellido,email,telefono,fax,user,es_admin,
   ifnull(group_concat(ac.curso_id),'') as cursos,sintesis
    from tbl_admin a
    left join tbl_admin_curso ac on a.id = ac.admin_id
    where a.id = '{$_REQUEST['ItemID']}'";
...
if (isset($_POST['login12'])) {
  ...
  $campos = explode(",", "nombre, apellido, user, es_admin, email, telefono, fax, fecha, sintesis");
